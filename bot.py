...
if not all([BOT_TOKEN, OWNER_ID, DATABASE_URL, RENDER_URL, UPLOAD_CHANNEL]):
    missing = [var for var in ['BOT_TOKEN', 'OWNER_ID', 'DATABASE_URL', 'RENDER_URL', 'UPLOAD_CHANNEL'] 
...
async def setup_bot_commands():
    ...
    owner_commands = commands + [
        types.BotCommand("upload", "Start file upload session"),
        types.BotCommand("setimage", "Set start/help image"),
        types.BotCommand("setmessage", "Set start/help message"),
        types.BotCommand("broadcast", "Broadcast message to all users"),
        types.BotCommand("stats", "View bot statistics"),
    ]
...
async def cmd_start(message: types.Message):
    ...
    if message_data and message_data['image_file_id']:
        ...
        await message.answer(
            message_data['text_content'] if message_data else "Welcome!",
            reply_markup=InlineKeyboardMarkup().add(
                InlineKeyboardButton("Help", callback_data="help_button")
            )
        )
...
async def cmd_help(message: types.Message):
    ...
    if message_data and message_data['image_file_id']:
        ...
        await message.answer(
            message_data['text_content'] if message_data else "Help information"
        )
...
@dp.message_handler(commands=['setimage'], user_id=OWNER_ID)
...
@dp.message_handler(state=MessageStates.waiting_for_message_type, user_id=OWNER_ID)
...
@dp.message_handler(commands=['setmessage'], user_id=OWNER_ID)
...
@dp.message_handler(state=MessageStates.waiting_for_message_text, user_id=OWNER_ID)
...
@dp.message_handler(state=BroadcastStates.waiting_for_broadcast, content_types=types.ContentType.ANY, user_id=OWNER_ID)
...
@dp.message_handler(state=UploadStates.waiting_for_files, content_types=types.ContentType.ANY, user_id=OWNER_ID)
...
@dp.message_handler(state=UploadStates.waiting_for_options, user_id=OWNER_ID)
async def process_upload_options(message: types.Message, state: FSMContext):
...
async def process_auto_delete(message: types.Message, state: FSMContext):
    ...
    for file_message in upload_sessions[user_id]:
        file_info = {
            ...
            'caption_entities': file_message.caption_entities,
        }
