# bot.py
"""
Final session-sharing Telegram bot (deploy-ready)

Features:
- aiogram 2.25.0, aiohttp-compatible (3.8.6)
- Async SQLAlchemy + asyncpg
- BigInteger columns for Telegram IDs (fixes int32 overflow)
- Upload flow that forwards files to a private upload channel and stores only file_ids + captions
- /upload, /done, /abort, /setprotect, /setautodelete, /revoke, /edit_start, /broadcast, /help
- Public /start (welcome) and /start <token> (deep-link downloads)
- Protect content (applies protect_content if supported) with owner bypass
- Autodelete messages after configured minutes; persisted schedule in DB
- /health endpoint for uptime checks
- Uses environment variables for configuration (no hard-coding)
- Designed for durability, small DB footprint, and concurrency
"""
...
OWNER_ID_ENV = os.environ.get("OWNER_ID")  # owner id as string (we'll convert)
DATABASE_URL = os.environ.get("DATABASE_URL")  # postgresql+asyncpg://...
UPLOAD_CHANNEL_ID_ENV = os.environ.get("UPLOAD_CHANNEL_ID")  # private channel id
...
async def ensure_tables_and_columns():
    ...
    Uses safe SQL 'ALTER TABLE IF EXISTS ...' where possible.
    """
    logger.info("Initializing DB and running auto migrations...")
    async with engine.begin() as conn:
        await conn.run_sync(Base.metadata.create_all)

        # Attempt to add columns if missing (Postgres supports IF NOT EXISTS for columns)
        try:
...
async def autodelete_worker():
    logger.info("Autodelete worker started; interval=%ss", AUTODELETE_CHECK_INTERVAL)
    while True:
        try:
            async with AsyncSessionLocal() as db:
                now = datetime.utcnow()
